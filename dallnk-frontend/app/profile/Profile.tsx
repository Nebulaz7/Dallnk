...
const ProfileBanner = ({ walletAddress, username, reputationScore }: ProfileBannerProps) => {
  const getBadgeColor = () => {
  ...
  const getBadgeIcon = () => {
    ...
    if (reputationScore >= 5000)
      return <HiOutlineShieldCheck className="w-5 h-5 text-[#ec4899]" />;
    if (reputationScore >= 1000)
      return <HiOutlineShieldCheck className="w-5 h-5 text-[#eab308]" />;
    if (reputationScore >= 500)
      return <HiOutlineShieldCheck className="w-5 h-5 text-[#94a3b8]" />;
    return <HiOutlineShieldCheck className="w-5 h-5 text-[#b45309]" />;
  };
...
const ActivityItem = ({ type, title, timestamp, status }: ActivityItemProps) => {
  ...
  const getTypeIcon = () => {
    switch(type) {
      case 'submission': return <HiOutlineChartBar className="w-5 h-5 text-[#6366f1]" />;
      case 'bounty': return <HiOutlineGift className="w-5 h-5 text-[#ec4899]" />;
      case 'reward': return <HiOutlineFire className="w-5 h-5 text-[#eab308]" />;
      default: return <HiOutlineTrendingUp className="w-5 h-5 text-[#10b981]" />;
    }
  };
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-[#1a1a2e] border border-white/10 rounded-lg hover:border-white/20 transition-all">
      ...
      <div className={`px-3 py-1 rounded-full text-xs border ${getStatusColor()}`}>
        {status}
      </div>
    </div>
  );
...
const BountyCard = ({ title, description, reward, submissions, deadline, status }: BountyCardProps) => {
  const getStatusColor = () => {
    switch(status) {
...
const ContentCard = ({ title, image, views, likes, comments, time }: ContentCardProps) => (
...
export default function ProfilePage() {
  ...
  const mockUser = {
    walletAddress: walletAddress || '0x1234567890abcdef1234567890abcdef12345678',
    username: walletAddress ? `User_${walletAddress.slice(2, 8)}` : 'Guest User',
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              />
              <StatCard 
                icon={<HiOutlineGift className="w-6 h-6 text-[#ec4899]" />}
